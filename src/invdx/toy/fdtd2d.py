...
four edges (PML is a later learning milestone — see toy/__init__.py).
"""
...
def line_flux_spectrum(line, freqs, dt, dx, sign=1.0):
    ...
    * dx. For an "x" line, power flowing toward +x is sign=-1 (Sx = -Ez*Hy);
    ...
    source and grid are physical (the toy's version of the pvgc lesson that
    normalization runs must share every numerical convention).
    """
    E, H = line["E"], line["H"]
    steps = E.shape[0]
