...
good label is beyond the existing grating_coupler CE figure of merit, or how the
resulting dataset should be split/normalized/consumed for training. Those are
research decisions that belong to whoever is using this module, expressed
through `kind`/`cfg`, not baked in here.
...
Three moving parts:
  ...
  generate_dataset(cfg, kind, n, run_dir, ...) -- runs one forward-only grating_coupler
      simulation per sampled design (through the same INVDX_FAST-gated
      forward loop `problems.grating_coupler.characterize` already uses — see
      `problems/grating_coupler.py`'s `_fdtd_forward`) and writes the result as npz
...
Currently wired to the grating_coupler problem only (`problems.grating_coupler`); a second problem
...
# `sample_designs` runs out of the box. A real use overrides them via the
# cfg._period_range_um / cfg._duty_range / cfg._rho_beta / cfg._rho_eta
# attributes (same "extra attribute on cfg" convention `problems.grating_coupler` uses
...
def _sample_uniform_grating(n, seed, cfg):
    ...
    grid with the same `rasterize_teeth` the optimizer's `--init grating`
    uses (see grating_coupler.rasterize_teeth for why pixel-rounding is not cosmetic)."""
    ...
    for _ in range(n):
        ...
        teeth = grating_coupler.uniform_grating_teeth(cfg, period, duty)
        yield grating_coupler.rasterize_teeth(cfg, teeth)
...
def _sample_random_rho(n, seed, cfg):
    ...
    cfg.filter_radius (== cfg.min_feature).  NOTE: an earlier wording here
    claimed the resulting feature size is "guaranteed"
    and matches "the fabrication rule the optimizer enforces".  Both are
    false: a length-scale guarantee needs a formulation this repo does not run
    (see Config.filter_radius), and the optimizer enforces no minimum feature
    at all -- min_feature is only MEASURED post-hoc by scripts/15 via
    fab.measure.min_feature_1d, and the linewidth check in docs/tolerance.md
    reports its number without ever failing on it.
    ...
    from .problems.grating_coupler import n_design_voxels
...
def _measure_one(cfg, rho, p_in_c, azimuth_sign, seed, lams_um=None):
    ...
    Uses `problems.grating_coupler.characterize`, NOT the differentiable `ce_from_arrays`
    ...
    meas = grating_coupler.characterize(cfg, teeth, p_in=p_in_c, azimuth_sign=azimuth_sign,
                             seed=seed)
...
def _readme_text(manifest):
    ...
    s = manifest["sampling"]
    lines = [
        "invdx grating_coupler forward-simulation dataset",
        "======================================",
        "",
...
def generate_dataset(cfg, kind, n, run_dir, shard_size=8, lams_um=None,
    ...
    # the optimizer measures it once for a whole optimization run.
    p_in_c, azimuth_sign, slope = grating_coupler.calibrated_beam(cfg, seed=seed)
    ...
    manifest = {
        ...
        "cfg": asdict(cfg),
        "n_design_voxels": grating_coupler.n_design_voxels(cfg),
        ...
        "units": {
            ...
            "rho": "material density in [0, 1] (binarized at 0.5 before "
                  "simulation, same convention as grating_coupler.profile_teeth)",
