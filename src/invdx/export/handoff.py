...
the run dir; anything skipped is recorded in manifest.json["notes"]):
    ...
    eps.npy         rasterized relative-permittivity grid of the design,
                    ...
                    (problems.grating_coupler.profile_teeth + eps_grid_xz)
...
importing the grating_coupler problem module only pulls in fdtdx/jax for its dataclass
...
from ..problems import grating_coupler

_COUPLER_FIELDS = {f.name for f in dataclasses.fields(grating_coupler.GratingCouplerConfig)}
_COUPLER_MARKERS = ("lam_c", "L_design", "design_grid_per_um", "t_si")
...
def _is_grating_coupler_config(cfg_dict):
    return bool(cfg_dict) and all(k in cfg_dict for k in _COUPLER_MARKERS)


def _grating_coupler_config_from_dict(cfg_dict):
    kwargs = {k: v for k, v in cfg_dict.items() if k in _COUPLER_FIELDS}
    if "beta_schedule" in kwargs:
        kwargs["beta_schedule"] = tuple(kwargs["beta_schedule"])
    return grating_coupler.GratingCouplerConfig(**kwargs)


def _rasterize_eps(cfg, rho_binary):
    """design_rho -> teeth -> permittivity raster, via the existing grating_coupler
    ...
    eps = grating_coupler.eps_grid_xz(cfg, teeth, nx, nz)
    return eps
...
def export_handoff(run_dir, out_dir=None):
    ...
    if os.path.exists(rho_path):
        ...
        # A rank-2 design carries y structure; every reader below assumes a
        # rank-1 one. n_design_voxels would silently become nx and the y
        # extent would vanish, giving a manifest that describes the design's
        # x-projection while looking complete. Refuse rather than export it.
        if np.ndim(rho) != 1:
            raise ValueError(
                f"design_rho.npy has shape {tuple(np.shape(rho))}: this "
                f"exporter is the 1D chain (teeth run-length -> extruded "
                f"rectangles) and would emit the x-projection of a 2D design "
                f"as if it were the whole thing. A 2D free-form design needs "
                f"the 2D export path, including axis names that distinguish "
                f"lateral y from vertical y.")
        ...
        if _is_grating_coupler_config(cfg_dict):
            cfg = _grating_coupler_config_from_dict(cfg_dict)
            ...
            eps_info = {
                ...
                "axis_order": ["x_um_propagation", "y_um_vertical"],
                "axis_note": ("grating_coupler coordinates: x = propagation direction "
                              "(grating/waveguide axis), y = vertical "
                              "(y=0 at the bottom of the silicon device "
                              "layer); both axes span the full simulation "
                              "cell, PML included. The axis names carry "
                              "their role because a bare 'y' is ambiguous in "
                              "this project: it is the VERTICAL axis in the "
                              "quasi-2D convention used here, and the LATERAL "
                              "axis in the 3D scene where a 2D free-form "
                              "design lives. A consumer that wires xi(x,y) "
                              "to this y would attach a lateral pattern to a "
                              "vertical axis, and the geometry would still "
                              "look plausible."),
            ...
            notes.append(
                ...
                "grating_coupler design (missing one of " + ", ".join(_COUPLER_MARKERS) +
                "); automatic rasterization is only implemented for the "
                "grating_coupler problem so far.")
    else:
