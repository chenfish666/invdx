...
hardcode numbers: everything tweakable lives here and is overridable per run
via `--set KEY=VAL` (see cli.py).
...
class BaseConfig:
    # ---- Numerics ----
    resolution: int = 80           # Meep-side resolution (pixels/um). Learned the
                                   # hard way: must be >= design_grid_per_um or adjoint
    ...
    def filter_radius(self):
        # WARNING: the identity below is TRUE but its
        ...
        # problems/grating_coupler.py or scripts/15 -- only in scripts/16, a post-hoc
        ...
        # from the other direction: its linewidth check is report-only, never
        # pass/fail, because a single nominal field carries no length-scale
        # guarantee.  Closing that gap means optimising the eroded and dilated
        # fields as well, which is a different formulation from the one this
        # repo implements.
