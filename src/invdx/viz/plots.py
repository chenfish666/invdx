...
# CJK-capable font stack (labels are Chinese; DejaVu alone renders tofu).
# addfont() registers the system Noto CJK explicitly — rcParams alone is not
# enough when matplotlib's font cache predates the font install.
...
def _peak_label(ax, lam, db):
    ax.annotate(f"峰值 {db:.2f} dB @ {lam:.3f} µm",
                xy=(lam, db), xytext=(8, 10), textcoords="offset points",
                fontsize=9, color=INK,
                arrowprops=dict(arrowstyle="-", color=INK2, lw=0.8))


def plot_ce_spectrum(res, out_png, title="CE 頻譜", pdf=False):
    """CE(λ) line(s): nominal + optional CD corners (07 verify results)."""
    fig, ax = _ax(title, "波長 (µm)", "CE (dB)")
    ...
    styles = {"erode_10nm": (C2, "侵蝕 −10 nm"), "dilate_10nm": (C3, "膨脹 +10 nm")}
...
def plot_history(res, out_png, title="優化軌跡", pdf=False):
    """Adjoint-optimization trace with damaged/intact reference levels."""
    fig, ax = _ax(title, "迭代", "mean T")
    h = res["history"]
    ax.plot(range(len(h)), h, color=C1, linewidth=2, label="_")
    for key, lab in (("T_intact", "完好"), ("T_damaged", "受損")):
        if key in res:
    ...
    if "T_healed" in res:
        ax.annotate(f"修復後 {res['T_healed']:.3f}",
...
def render_run(run_dir, out_dir=None, pdf=False):
    ...
    if os.path.exists(gp):
        ...
        made.append(plot_transmission(
            ...
            "光子晶體能隙(bulk 透射)", gap_ref=(0.29, 0.41), pdf=pdf))
    ...
    for fz in sorted(glob.glob(os.path.join(run_dir, "field_*.npz"))):
        ...
        if "extent" in z:
            ...
            ext = None
        title = str(z["title"]) if "title" in z else "穩態場 Re Ez"
