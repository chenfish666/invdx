"""Cross-engine convention contracts — pvgc's hard-won lessons as code.

Every rule here cost real debugging time in pvgc (see the predecessor PVGC study's lab log,
2026-07-12). Gates and engine adapters import from this module instead of
re-deriving conventions ad hoc; that is the whole point.
...
# Verified against fiber-side reciprocity in pvgc (2.93 dB -> 0.150 dB).
...
def collapse_multifreq_gradient(dJ):
    ...
    (pvgc bug: ravel silently produced a wrong-length gradient.)"""
    dJ = np.asarray(dJ)
    if dJ.ndim == 2:
...
def assert_resolution_covers_design_grid(cfg):
    ...
    resolution is below the design grid density (pvgc: 5-8% error at res 40,
    exact at res 80 with design_grid_per_um=100). Fail loudly, don't warn."""
    if cfg.resolution < cfg.design_grid_per_um:
        raise ValueError(
            ...
            f"systematically underestimated (pvgc measured 5-8% at res 40). "
...
# ~3.4x slower than 1e-6 with FOM unchanged to 4 decimals (pvgc measured).
# meep_worker must always pass cfg.dft_decay_tol explicitly.
MEEP_DECAY_BY_DEFAULT_TOO_SLOW = 1e-11


# Lesson 5 — FOM spectral sampling vs validation density (pvgc v2, 2026-07-12).
# A minimax FOM over sparse wavelength samples lets the optimizer pump the
# sampled points while the spectrum collapses between them (pvgc v2: 3 points
# over 100 nm -> 1330 nm valley at -26.8 dB while samples reached -21 dB, peak
# migrated to the band edge). Guards: (a) FOM sample spacing must be finer
# than the device's expected spectral feature width (grating: ~bandwidth/4);
# (b) never claim a bandwidth wider than the physics supports; (c) always
# validate on a dense spectrum before trusting an optimized design — that
# validation belongs in the problem's post-optimization gate.
def assert_fom_sampling_covers_band(sample_spacing_nm, expected_feature_nm):
    if sample_spacing_nm > expected_feature_nm / 2:
        raise ValueError(
            ...
            f"the optimizer can exploit the gaps (pvgc v2 pathology). "
            f"Densify samples or narrow the band.")


# Lesson 6 — cross-engine comparisons of resonant structures must compare
# spectra, not single-wavelength values (invdx res-80 incident, 2026-07-13).
...
# edges (Meep Block) differ by up to half a cell per edge. The PVGC uniform
# grating's coupling ridge moves ~2.4 nm of peak wavelength per nm of tooth
# width, so a half-pixel edge difference (6.25 nm at res 80) shifts the ridge
# by ~15-30 nm and a fixed-wavelength CE reading swings by ~20 dB while the
# ridge PEAK stays put (measured: fdtdx duty-0.500 peak -8.89 dB @1.340um,
# duty-0.478 peak -8.84 dB @1.310um, pvgc/Meep -10.1 dB @1.310um).
# Corollaries: (a) validate cross-engine on ridge position+peak, with the
# effective-geometry uncertainty stated; (b) this hypersensitivity IS the
# fabrication-tolerance problem the toolbox exists to attack.
