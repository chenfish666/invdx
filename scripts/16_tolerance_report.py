#!/usr/bin/env python
"""M1+ tolerance report for a finished pvgc optimization run (docs/tolerance.md
"Planned use: M1+"): a per-voxel sensitivity map and a three-corner robust-
design evaluation. No re-optimization; reads only run_dir/config.json and
run_dir/opt_state.npz (the checkpoint format optimize.py already writes) and
writes into run_dir/tolerance/.

  python scripts/16_tolerance_report.py runs/<pvgc-opt-dir>
  python scripts/16_tolerance_report.py runs/<dir> --lams 1.27,1.35,9

Sensitivity map (tolerance.md M1+ #1): one backward pass of the existing
`pvgc.make_ce_value_and_grad` FOM, evaluated at the run's own final (p, beta)
...
Corner evaluation (tolerance.md M1+ #2): the SAME latent p, re-projected at
...
scripts/15 uses for design_rho.npy) and measured with pvgc.characterize.
...
def load_run(run_dir):
    """(cfg, p_flat, beta, iteration) from an existing pvgc-opt run dir."""
    ...
    if not os.path.exists(os.path.join(run_dir, optimize.STATE_FILE)):
        raise SystemExit(f"{run_dir} has no {optimize.STATE_FILE} -- run "
                         f"scripts/15_pvgc_optimize.py first")
...
def corner_table(cfg, p_flat, beta, p_in_c, azimuth_sign, lams_spec=None,
    ...
    for name, eta_attr in CORNER_DEFS:
        ...
        row = {"corner": name}
        if lams_spec:
            spec = pvgc.characterize_spectrum(cfg, teeth, lams_spec,
                                              azimuth_sign=azimuth_sign,
                                              seed=seed)["spectrum"]
            peak = max(spec, key=lambda r: r["CE_dB"])
            bw, lam_lo, lam_hi, note = pvgc.bandwidth_3db(spec)
...
def main():
    ...
    p.add_argument("run_dir",
                   help="finished pvgc-opt run directory (read-only, "
    ...
    p_in_c, azimuth_sign, _ = pvgc.calibrated_beam(cfg)
