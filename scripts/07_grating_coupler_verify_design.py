#!/usr/bin/env python
"""Independent-engine verification of a grating_coupler design (the reason invdx has a
second engine): load a grating_coupler run's binarized design profile, re-measure it on
fdtdx, and report the quantities a results table needs — CE spectrum, ridge,
linewidth, and optionally the CD-corner sensitivity.

  python scripts/07_grating_coupler_verify_design.py --run runs/<dir> \
      [--set spacing_um=0.010] [--corners] [--tag verify]

Reads ONLY from the grating_coupler run dir (never writes there). The grating_coupler config in
that dir supplies theta/band/design-grid; every value can still be
overridden with --set.

Grid choice: default spacing is 10 nm = exactly the grating_coupler design grid, so
tooth edges land ON fdtdx cell boundaries and the effective-duty
discretization error (conventions lesson 6; the ridge wavelength shifts
measurably with linewidth) is zero by construction.

Cross-engine acceptance is SPECTRAL: compare the CE(lambda) ridge shape and
peak against the optimizer's own binarize-validation numbers, never a single
wavelength.
"""
...
def load_grating_coupler_design(run_dir, rho_name="design_rho.npy"):
...
def split_dual(cfg, rho_a, rho_b):
    """grating_coupler dual-etch convention (sims.profile_geometry_dual): full-height
    ...
    return grating_coupler.profile_teeth(cfg, full), grating_coupler.profile_teeth(cfg, shallow)
...
def verify(cfg, rho_binary, lams_um, corners=False, s11=False,
    ...
    B = rho2_binary, grating_coupler convention)."""
    solid, void = measure.min_feature_1d(rho_binary, cfg.design_grid_per_um)
    if rho2_binary is None:
        teeth = grating_coupler.profile_teeth(cfg, rho_binary)
        shallow = None
    else:
    ...
    p_in, azimuth_sign, _ = grating_coupler.calibrated_beam(cfg)
    spec = grating_coupler.characterize_spectrum(cfg, teeth, lams_um,
                                      azimuth_sign=azimuth_sign,
                                      shallow_teeth=shallow)
    out["spectrum"] = spec["spectrum"]
    out["peak"] = max(spec["spectrum"], key=lambda r: r["CE_dB"])
    bw, lam_lo, lam_hi, bw_note = grating_coupler.bandwidth_3db(spec["spectrum"])
    ...
    if s11:
        ...
        ws = grating_coupler.wg_side_characterize(cfg, teeth, shallow_teeth=shallow)
    ...
    if corners:
        ...
        for name, delta in (("erode_10nm", -0.010), ("dilate_10nm", +0.010)):
            ...
            if rho2_binary is None:
                teeth_c, shallow_c = grating_coupler.profile_teeth(cfg, rho_c), None
            else:
            ...
            spec_c = grating_coupler.characterize_spectrum(cfg, teeth_c, lams_um,
                                                azimuth_sign=azimuth_sign,
    ...
    if energy_budget:
        ...
        # grating_coupler.energy_budget for the five measurement judgments this codifies
        out["energy_budget"] = grating_coupler.energy_budget(
...
def main():
    ...
    p.add_argument("--run", required=True, help="grating_coupler run dir (read-only)")
    ...
    p.add_argument("--energy-budget", action="store_true",
                   help="also compute the box-face energy budget (where "
                        ...
                        "simulation) — see grating_coupler.energy_budget")
    ...
    rho, pcfg = load_grating_coupler_design(args.run, args.rho_name)
    ...
    d = start_run(cfg, args, "coupler-verify")
    ...
    if args.field:
        if rho2_binary is None:
            teeth, shallow = grating_coupler.profile_teeth(cfg, rho_binary), None
        else:
            teeth, shallow = split_dual(cfg, rho_binary, rho2_binary)
        fm = grating_coupler.field_map(cfg, teeth, shallow_teeth=shallow)
    ...
    if args.field_3d:
        ...
        cfg3 = grating_coupler.GratingCouplerConfig(spacing_um=0.575 / 16, sim_time_s=0.8e-12,
                               theta_deg=cfg.theta_deg)
        cfg3.design_grid_per_um = cfg.design_grid_per_um
        teeth = grating_coupler.profile_teeth(cfg3, rho_binary)
        fm3 = grating_coupler.field_map_3d(cfg3, teeth, wg_width_um=args.width)
