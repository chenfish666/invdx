#!/usr/bin/env python
"""Checkpoint count C vs wall-clock time and memory, on the grating_coupler FOM.
...
docs/optimize.md), all measured at the reference recipe (spacing_um=0.020,
...
This script's DEFAULTS reproduce that recipe on purpose: a sweep run
...
For each requested C this builds `grating_coupler.make_ce_value_and_grad(cfg,
...
* Quasi-2D only. `cfg.n_y_cells` (default 4) makes every scene here a thin
  periodic-y slab, exactly like the rest of this repo's grating_coupler problem. A real
  3D scene's bytes/cell and time/checkpoint are NOT validated here — this
  measures the grating_coupler problem this repo actually runs, not a 3D projection of
  it.
...
# 5.93/11.25/15.42 GB) were measured at (docs/optimize.md). This script's
...
def anchor_comparable(cfg):
    ...
    (C=10/20/28 -> 5.93/11.25/15.42 GB, docs/optimize.md) were measured
...
def make_config(args):
    """Reference-recipe defaults (spacing_um=0.020, design_grid_per_um=50,
    ...
    cfg = grating_coupler.GratingCouplerConfig(**ANCHOR_CONDITION)
...
def main():
    ...
    if not comparable:
        print("[sweep] WARNING: cfg does not match the anchor condition "
              ...
              "(docs/optimize.md), see results.json anchor_comparable",
              flush=True)
