#!/usr/bin/env python
"""First 3D validation of the grating_coupler problem on GPU (grating_coupler validate_3d
equivalent): extrude the uniform grating to width W, radial Gaussian beam,
CE into the approximate fundamental waveguide mode.

Expectation to check the run against: 3D CE is LOWER than the quasi-2D
result at the same theta, because the quasi-2D scene cannot lose power to
lateral diffraction. The waveguide mode is a separable TE0(z)*cos(y)
approximation — stated in the output.

Long run: launch detached (setsid nohup) — see Makefile target or:
  setsid nohup env CUDA_VISIBLE_DEVICES=0 <python> scripts/04_grating_coupler_3d.py \
...
# the resulting 62.7M cells fit one 48 GB GPU, while 25 nm spacing (187M
# cells) does not
...
def main():
    ...
    print(f"[3d] ridge peak: {pk['CE_dB']:.2f} dB @ {pk['lam_um']:.3f} um "
          f"(expect below the quasi-2D ridge: lateral diffraction loss)  "
          f"[{res['wall_s']}s]")
    print(f"[3d] {res['note']}")
