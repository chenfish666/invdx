#!/usr/bin/env python
"""Dual-GPU 3D PVGC validation: the empty-cell normalization run and the
...
each card hosts exactly one ArrayContainer (no dual-container OOM).
  ...
  python scripts/05_pvgc_3d_dual.py --tag dual3d          # orchestrator
...
def worker(args):
    ...
    teeth = (None if args.role == "empty" else
             pvgc.uniform_grating_teeth(cfg, args.period, args.duty))
    planes = pvgc.run_3d_planes(cfg, teeth, args.width,
...
def orchestrate(args, argv):
    d = start_run(make_cfg(args), args, "pvgc-3d-dual")
    t0 = time.time()
    procs = []
    ...
    cfg = make_cfg(args)
    res = pvgc.combine_3d(outs["empty"]["fiber"], outs["grating"]["wg"],
