> **English** · [繁體中文](optimize.zh-TW.md)
...
the coupling-efficiency (CE) chain, so one backward pass reaches all 500
design variables.
...
measured 2.5e-6 dB apart (20 nm grid, fibre incidence angle theta=10 deg,
acceptance threshold 0.05 dB; recorded under the first-production-driver entry in
...
reproduces the cross-validated grating exactly. Guardrail 6 under
"Hard-won guardrails" in the main README (spectral ridges, not
single-wavelength values) bites inside a single engine too.
...
FOM is a jnp twin of the TE0 coupling-efficiency overlap used
...
(`GRADCHECK_MIN_REL_GRAD`, called the *signal floor* throughout — voxels
below it are dominated by tanh-saturation float32 noise regardless of the FD
method, so loosening the tolerance there would hide nothing informative). It runs **once, upfront**,
...
check.** The first production-scale run (0.8 ps, theta=10) failed
...
either (15% of peak gradient, 3x the signal floor). Root cause: FD
...
card, and `"reversible"` is worse here because recording the PML (perfectly
matched layer, the FDTD absorbing boundary) for 20k steps costs hundreds of
GB). `invdx.optimize` writes `opt_state.npz`
