...
facts (driver version, ports, hostnames) never appear here — see the local
operator's own notes for those.
...
- `spack/tools/` — Lmod and Apptainer, the cluster-user-space layer. Kept
  ...
  are frozen (not touched by this document's own history; see `git log` on
  that path).
...
uv run make gates GPU=0    # G0-G5, all green, G5 not skipped
```
...
changed relative to upstream (commit `614f57b`):
...
*intent* ("I want meep 1.34 with these variants"); `spack.lock` records the
...
project bumped to meep 1.34.0 (commit `614f57b`) — rebuilding confirmed the
upstream fix, meep built clean against whatever current swig the
concretizer picked (4.4.1).
