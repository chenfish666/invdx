# 親手重現:光子晶體 90° 彎(實驗室血脈基準)

這份教學帶你**親手**重現教授的論文
*"Transmission characteristic of defect for the 90-degree bend photonic
crystal"*(Lu et al.;已發表之實驗室論文):二維方晶格介電柱光子晶體、
90° 彎波導、點缺陷對透射的影響。全程用你自己專案裡的兩個引擎——
`toy/`(你可以逐行讀懂的 ~120 行 FDTD)和 Meep(領域公認錨點)。

**論文參數**(全部寫在 `PhCBendConfig`,沒有任何數字藏在腳本裡):
...
| R | 0.225a | 介電柱半徑 |
| ε | 10(n≈3.162) | 柱的介電常數 |
| 能隙 | f = 0.29–0.41 | 歸一化頻率 f = a/λ(論文宣告值,我們的錨點) |
| 偏振 | E 沿柱軸 | ⚠️ 見下方命名陷阱 |

> **命名陷阱(先讀這個)**:論文說它打入「TE」光,但柱狀晶格的能隙
> 屬於 **E 平行柱軸** 的偏振——物理教科書(Joannopoulos)稱之為
> **TM**,工程文獻(含這篇論文)常反過來叫 TE。我們的 toy 引擎演化
> (Ez, Hx, Hy)正是 E 沿柱軸,所以偏振是對的;名字不要背,記「E
> 沿著柱子」就永遠不會錯。這是跨文獻比對時最常見的一種慣例地雷,
> 和 `engines/conventions.py` 記錄的 ½ 功率慣例是同一類故事。

每一步都是一條指令。預設參數 = 論文尺寸(21×21),單步約 30–60 秒;
...
- `--set res_per_a=30`:離散化更細,下緣應更貼近論文值。
...
三種方位),看能隙內平均透射掉多少。論文結論:**垂直與水平缺陷的
影響遠大於斜角缺陷**。你的 delta 排序有沒有重現這個結論?

## 第 6 步(展望):從這裡到逆向設計

論文時代的做法:人工挑幾個缺陷位置逐一模擬(或 shift-or-shrink
啟發式)。逆向設計的做法:把每根柱的位置/半徑當可微參數,伴隨法
一次算出全部梯度。同一個彎、同一個 FDTD,方法演進了二十年——
這就是你論文裡「站在實驗室肩膀上」的那一頁。toy 引擎的下一個
里程碑(JAX 化 + jax.grad)會讓你在這個最簡單的結構上親手寫出
第一個伴隨梯度。
