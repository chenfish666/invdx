"""Unit tests for the paper toolkit: 3dB bandwidth math, viz rendering,
...
def test_bandwidth_3db_interpolated():
    ...
    bw, lo, hi, note = pvgc.bandwidth_3db(_spec(lams, ces))
    assert note is None
...
def test_bandwidth_3db_clipped_is_honest():
    ...
    lams = np.arange(1.27, 1.351, 0.01)
    bw, lo, hi, note = pvgc.bandwidth_3db(_spec(lams, np.full(len(lams), -10.0)))
