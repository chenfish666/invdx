...
Contracts pinned here:
  ...
  * the traced aux {"ce", "s11"} agrees with the INDEPENDENT numpy
    measurement chain (grating_coupler.wg_side_characterize) on the same grid-aligned
...
from invdx.problems.grating_coupler import GratingCouplerConfig, n_design_voxels
...
def _tiny_cfg(**overrides):
    """Same tiny quasi-2D grating_coupler scene as test_tolerance_report.py::_tiny_cfg
...
def legacy(p0):
    ...
    vg_fn, _, _, _, _, value_fn = grating_coupler.make_ce_value_and_grad(
...
def test_w_s11_defaults_to_float_zero_and_casts_from_cli():
    assert type(GratingCouplerConfig().w_s11) is float
    assert GratingCouplerConfig().w_s11 == 0.0
    cfg = apply_overrides(GratingCouplerConfig(),
...
def test_wg_ce_vs_legacy_ce_logged_not_asserted(wg, legacy):
    """The wg CE term relies on reciprocity, which only closes on a
    converged, production-scale scene; on this transient tiny scene the
    delta is uncontrolled, so it is logged for the record, never asserted."""
...
def test_wg_gradient_matches_finite_difference(wg, p0):
    ...
    checkpointed backward pass (the open risk this gate exists for) —
    surface it, do not tune the tolerance.

    It uses TWO independent random directions, because one direction can
    hide a wrong gradient component by chance.

    Gate history — READ BEFORE "fixing" a future failure here:
    an earlier version of this gate additionally required an h-refinement
...
def test_richardson_gate_negative_control_rejects_perturbed_adjoint(wg, p0):
    ...
    while providing none (this "zero-sensitivity check" shape has turned up
    in this project more than once). But a negative
...
# --------------------------------------------------------------------------
# t6 — independent numpy ground truth for the traced S11 and CE: the t3
#      identity is true by construction, so only a chain that never touches
#      the traced code can certify what R11 IS
# --------------------------------------------------------------------------
...
def xcheck():
    ...
    chains: grating_coupler.wg_side_characterize (its own run, its own numpy overlap
    ...
    pixels Si) is the source of truth:
      ...
      * numpy chain: profile_teeth(rho) as static UniformMaterialObject
        ...
        pinned in tests/test_grating_coupler_optimize.py). The design pixel (1/20 um)
    ...
    rho = grating_coupler.rasterize_teeth(
        cfg, grating_coupler.uniform_grating_teeth(cfg, period=0.6, duty=0.5))
    ...
    try:
        num = grating_coupler.wg_side_characterize(cfg, teeth)
    finally:
        if prev is None:
    ...
    vg_fn, _, _, _, _, _ = grating_coupler.make_ce_value_and_grad(
