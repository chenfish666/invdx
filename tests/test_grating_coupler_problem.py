"""Pure-math tests for the grating_coupler problem layer (no simulation, no GPU)."""
...
def test_te0_neff_bracketed_and_consistent():
    neff = grating_coupler.slab_te0_neff(CFG.lam_c, CFG.t_si, CFG.n_si, CFG.n_sio2, 1.0)
...
def test_mode_field_continuous_and_decaying():
    ...
    Ey, Hz, neff = grating_coupler.slab_te0_mode(zs, 0.0, CFG)
...
def test_teeth_builders():
    ...
    t2 = grating_coupler.profile_teeth(CFG, rho)
    assert len(t2) == 1
