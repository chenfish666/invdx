"""End-to-end tests for scripts/16_tolerance_report.py (the fabrication
tolerance report): per-voxel sensitivity map + three-corner robust-design
evaluation.

CPU, a tiny quasi-2D grating_coupler scene -- the same coarse-grid/short-sim-time
...
(grating_coupler.assert_design_grid_snaps) requires t_si to be an integer multiple of
...
scripts/15_grating_coupler_optimize.py: that checkpoint pair is the entire contract
...
def _tiny_cfg(**overrides):
    """Same tiny quasi-2D grating_coupler scene as test_datasets.py::_tiny_cfg, plus
    t_si=0.20 (see module docstring): 0.220 (the GratingCouplerConfig default) is not
...
def _load_tolerance_script():
    ...
    `import`), same trick tests/test_grating_coupler_optimize.py uses for script 15."""
...
def _make_run_dir(tmp_path, cfg):
    run_dir = str(tmp_path / "coupler-opt-tiny")
...
def _run_main(argv_tail):
    ...
    already covered elsewhere (test_grating_coupler_optimize.py); it does not change
...
def test_corner_table_with_lams_fills_bandwidth_and_ridge(run_dir):
    ...
    p_in_c, azimuth_sign, _ = mod.grating_coupler.calibrated_beam(cfg)
