...
-65 dB phantom (upward target written with the downward H sign) and pvgc's
...
def test_upward_target_accepts_upward_wave():
    ...
    Eg, Hg = pvgc.gaussian_mode_tilted(xs, 0.0, W0, LAM, TH, kx_sign=-1.0)
    P = pvgc.overlap_power_directional(E, H, Eg, Hg, dx)
...
def test_upward_target_rejects_downward_wave():
    ...
    dx = xs[1] - xs[0]
    E, H = _upward_field(xs, kx_sign=-1.0)
    P_up = pvgc.overlap_power_directional(
        E, H, *pvgc.gaussian_mode_tilted(xs, 0.0, W0, LAM, TH, -1.0), dx)
    # downward wave: flip H sign
    P_down = pvgc.overlap_power_directional(
        E, -H, *pvgc.gaussian_mode_tilted(xs, 0.0, W0, LAM, TH, -1.0), dx)
...
def test_wrong_kx_sign_discriminated():
    ...
    dx = xs[1] - xs[0]
    E, H = _upward_field(xs, kx_sign=-1.0)
    P_match = pvgc.overlap_power_directional(
        E, H, *pvgc.gaussian_mode_tilted(xs, 0.0, W0, LAM, TH, -1.0), dx)
    P_wrong = pvgc.overlap_power_directional(
        E, H, *pvgc.gaussian_mode_tilted(xs, 0.0, W0, LAM, TH, +1.0), dx)
